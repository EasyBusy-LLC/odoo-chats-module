...
class EasyBusyCreateOpportunity(models.TransientModel):
    ...
    def _resolve_partner(self):
        ...
        if self.customer_action == "create":
            ...
            if "mobile" in partner_model._fields:
                partner_vals["mobile"] = self.contact_phone
            ctx = self.env.context.copy()
            if payload:
                ctx["easy_busy_chat_link_payload"] = payload
            return partner_model.with_context(ctx).sudo().create(partner_vals)
    ...
    def _prepare_lead_vals(self, partner):
        ...
        if "mobile" in lead_model._fields:
            lead_vals["mobile"] = self.contact_phone
    ...
    def _get_duplicate_opportunities_for_values(self, partner_id=False, phone=False, email=False):
        ...
        for field_name, phone_candidate in self._get_crm_lead_phone_candidates(search, phone):
            leads |= search.search(
                [
                    ("type", "=", "opportunity"),
                    (field_name, "=", phone_candidate),
                ]
            )
    ...
    @staticmethod
    def _get_crm_lead_phone_fields(lead_model):
        return [
            field_name
    ...
    def _get_crm_lead_phone_candidates(self, lead_model, phone):
        candidates = []
        for phone_candidate in self._phone_search_candidates(phone):
            for field_name in self._get_crm_lead_phone_fields(lead_model):
                candidate = (field_name, phone_candidate)
                if candidate not in candidates:
                    candidates.append(candidate)

        sanitized_phone = self.env.user._phone_format(number=phone)
        if sanitized_phone and "phone_sanitized" in lead_model._fields:
            candidate = ("phone_sanitized", sanitized_phone)
            if candidate not in candidates:
                candidates.append(candidate)
